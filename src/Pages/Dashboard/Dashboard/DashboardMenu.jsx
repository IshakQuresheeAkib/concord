...
const DashboardMenu = () => {
    ...
    return (
        <nav className="relative w-full h-full flex flex-col justify-between overflow-y-auto custom-scrollbar bg-white/90 backdrop-blur-md transition-all duration-500 font-Nunito">
            ...
            <div className="space-y-6 mt-8 md:mt-10 relative z-10 px-4 md:px-6">
                
                {/* Profile Section */}
                <div id="profile" className="flex flex-col items-center space-y-3 group shrink-0">
                    <div className="relative">
                        ...
                        <img 
                            src={photoURL} 
                            alt="Avatar user"
                            className="relative w-20 h-20 md:w-24 md:h-24 rounded-full object-cover ring-2 ring-teal/50 ring-offset-4 ring-offset-white transition-transform duration-300 group-hover:scale-105"
                        />
                    </div>
                    <div className="text-center relative">
                        <div className="inline-block px-3 py-1 mb-1.5 text-[10px] tracking-widest text-teal bg-teal/10 border border-teal/30 rounded-full uppercase">
                            {isAdmin ? 'System Admin' : 'Authorized User'}
                        </div>
                        <h2 className="font-semibold text-sm md:text-base text-black/80 drop-shadow-sm line-clamp-1">
                            {displayName}
                        </h2>
                        <p className="text-xs text-black/50 line-clamp-1">{email}</p>
                    </div>
                </div>
                        ...
                        <NavLink 
                            ...
                            className={({ isActive }) => `
                                ...
                                ${isActive 
                                    ...
                                    : 'text-black/60 hover:text-teal hover:bg-gray/80 p-3'}
                            `}
                        >
                            ...
                            <span className="text-lg md:text-xl transition-transform duration-300 group-hover:scale-110 group-hover:-rotate-3 drop-shadow-sm">
                                {navItem.icon}
                            </span>
            ...
            {/* Logout Section */}
            <div className="relative z-10 p-4 md:p-6 pb-6 shrink-0 mt-auto bg-white/50 backdrop-blur-sm sticky bottom-0">
